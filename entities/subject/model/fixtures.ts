    ...
    rules: [
      {
        ...
        label: "Используйте семантические элементы",
        description:
          "Используйте семантические теги вроде <header>, <main>, <section>, <article>, <nav> для доступности и SEO. Избегайте обобщённых <div>, когда есть подходящий семантический тег.",
      },
      {
        id: "alt-text",
...
| toSpliced | Создаёт копию массива с изменениями, не мутируя исходный          | arr.toSpliced(start, deleteCount, ...items)  |  const arr=[1,2,3]; const sp=arr.toSpliced(1,1,9); // sp=[1,9,3]             |
      ...
      {
        id: "collections",
        label: "Map, WeakMap, Set",
        description:
          "Map: ключ→значение (любой ключ). WeakMap: только объектные ключи (дружит со сборщиком мусора). Set: уникальные значения — [...new Set(arr)] для дедупликации.",
      },
      {
        id: "promises",
        label: "Promise",
    ...
    rules: [
      {
        ...
        description: "DOM → CSSOM → Render Tree → Style → Layout → Paint → Compositing.",
      },
      {
        id: "selectors",
    ...
    rules: [
      {
        id: "rest",
        label: "REST",
        description: `REST - это способ создания API с помощью протокола HTTP;
6 принципов: 
